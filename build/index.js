...
const retornoEl = document.getElementById("feedback");
const barraProgresso = document.getElementById("progress-bar");
const textoProgresso = document.getElementById("progress-text");
const pctProgresso = document.getElementById("progress-pct");
const botoesFiltrо = document.querySelectorAll(".btn-filtro");
...
function carregarDados() {
    ...
    if (salvo) {
        ...
        tarefas = (_a = parsed.tarefas) !== null && _a !== void 0 ? _a : [];
        proximoId = (_b = parsed.proximoId) !== null && _b !== void 0 ? _b : tarefas.length + 1;
    }
}
function salvarDados() {
    localStorage.setItem(CHAVE_STORAGE, JSON.stringify({ tarefas, proximoId }));
}
function mostrarRetorno(mensagem, tipo = "erro") {
...
function excluirTarefa(id) {
    const li = listaEl.querySelector(`[data-id="${id}"]`);
    if (li) {
